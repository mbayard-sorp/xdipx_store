...
export const IVR_LIMITS = {
  ...
  initialSilenceMs: Number(process.env['IVR_INITIAL_SILENCE_MS'] ?? 8000),
  /** Between caller turns (after Claude finishes replying). */
  interTurnSilenceMs: Number(process.env['IVR_INTER_TURN_SILENCE_MS'] ?? 12000),
  ...
  reEngageAttempts: Number(process.env['IVR_RE_ENGAGE_ATTEMPTS'] ?? 1),
