...
- **The only real metered cost is images, and underspending on them is also a failure.** A day generating a few images ≈ $0.10–0.50 (via `generateImage()`, routing per `docs/media-model-routing.md`), which the daily cap comfortably absorbs. Do not treat a $0 image day as a win: on a changed-hero or changed-theme day it is a fresh-art-floor failure. The $/day cap bounds runaway loops and the weekly design cycle, not ordinary daily art.
...
- **Hard `maxTurns`.** The routine has a turn cap; the canonical number lives in `docs/homepage-team/routine-daily-merchandise.md` (~36–40 as of the 2026-08-16 owner decision — this file previously said ~12–16, which was stale twice over and is why the number now lives in one place). If you're looping without converging, stop and report rather than burning turns. Never re-run yourself.
...
7. Sequence specialists: `emma-copywriter` (proposes brand-fit candidates + copy, gated by `emma-empathy-reviewer`) → `homepage-cro` (the pick gate: scores candidates on the lead-surface price floor (mission brief §5: anchors ≥ $39 on hero/first rail/default finder results; consumables attach-only, never the hero or first-rail slots 1-3), margin (msrp minus wholesale_cost), price-point spread across rails, deal_score, and stock depth; nothing ships with unknown margin, and never a MAP=MSRP product on a discount-styled surface) → `homepage-art-director` (the day's visual scheme + per-slot prompt briefs) → `media-manager` (generate to the fresh-art floor, reuse as fallback) → write Sanity homepage doc + Shopify metafields (diff-before-write).
...
- Imagery → `media-manager` (fresh-art floor on homepage art, generated via `generateImage()` with Atlas primary per `docs/media-model-routing.md`; reuse-first for packshots and PDP art).
