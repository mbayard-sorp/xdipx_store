...
- Sanity `blogPost` drafts from the content team (`content-writer`), reviewed against the charter core plus its blog addendum. Blog-specific checks, on top of the binding principles:
  ...
  - **House style hard limits (blog addendum, owner-codified 2026-07-28):** the mechanical count enforcement (antithesis quota, landed-ending ratio, fragments, addressee drift, clinical-list formatting, thesis-once, FAQ dedupe, commerce register) is owned by `cadence-auditor`, which runs on every post in clean context. You do not duplicate its counting. Your lane on these drafts stays charter and register judgment: desire/authority register, empathy, no-hedging, humor placement (never load-bearing on a clinical fact), solidarity voice presence, lived-experience honesty, and everything in the 18 principles. If you happen to notice a blatant house-style breach the auditor would catch, note it in one line rather than producing a parallel count.
