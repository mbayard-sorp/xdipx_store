...
- Sanity `blogPost` drafts from the content team (`content-writer`), reviewed against the charter core plus its blog addendum. Blog-specific checks, on top of the binding principles:
  ...
  - **Blog addendum compliance:** structure by category (answer-shaped for guides-family, essay-shaped with a direct-answer capsule for real-talk), no medical claims, no prices or discount claims in body text, inclusive wellness tone, AI-guide authorship honesty.
  - **House style hard limits (blog addendum, owner-codified 2026-07-28), enforced with counts:** independently count (a) antithesis constructions ("X, not Y" and near-variants; quota 3/article, section-closing), (b) paragraphs ending on a landed line (max 1 in 3), (c) fragments (max 5, never 2+ consecutive); verify one addressee per section with explicit switches, clinical/safety criteria rendered as a bulleted list with bolded lead-in (prose red-flags are a REVISE), thesis stated in full exactly once, every FAQ answer adding beyond the body (verbatim body-phrase reuse is a REVISE), commerce mentions in the surrounding register with no editorial-list references, and no claims added for rhythm. Verify the writer's reported counts; a count over quota is a REVISE with the offending instances listed.
