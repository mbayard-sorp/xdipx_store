...
- **A resolved contact is required on every pitch row, not just a resolved target (ticket #10495).** Measured 2026-09-20: only 4 of 21 approved outreach rows carried a pasteable email address; 14 had finished, voice-gated copy but no address (9 of those named no contact path at all), and the queue had not moved since 2026-08-21 because "send it" was never actually true. Before filing an OFFSITE PITCH or BRAND-PARTNER OUTREACH row, resolve either (a) a specific email address, or (b) a named submission URL plus the current editor or masthead role it targets, with the date you verified it, and file the contact on the row. A target that cannot clear that bar is not a pitch: file it labeled `LEAD` instead (same research, no send-ready copy implied), so the send queue and the research queue stop being the same list. Never ship a literal placeholder like "(editor name)" in a greeting: resolve the name at filing time or leave the row a LEAD. Applies to duties 2 and 6 alike, and a LEAD row counts against the same per-run cap as a pitch row.
...
2. **Pitch drafts**: for the 3-5 best targets, draft a short, honest pitch (who we are, why we fit their list, what makes xdipx different) ready for the owner to send from hello@xdipx.com. One suggestion row each, kind `strategy`, with the target URL, resolved contact path, draft copy, and the policy note. No resolved contact per the hard rule above: file the row as a `LEAD` instead, contact research still to do.
...
6. **Brand-partner outreach**: each run, pick 2-3 brands xdipx actually carries (Shopify vendor field is the source of truth), find their partner/affiliate/where-to-buy/stockist pages and marketing contact, and draft an intro pitch (who xdipx is, that we carry and editorially feature their products, and an ask to be listed as a stockist or included in their where-to-buy, or a reciprocal link or social mention). Same suggestion-row format, resolved-contact requirement, and policy note as the OFFSITE PITCH rows; still propose-only.
