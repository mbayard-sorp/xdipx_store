...
- Timeout risk: `vercel.json` sets `maxDuration: 300`, and a full-catalog recompute (serial Shopify calls over the current catalog) takes 4–5 minutes, so 300s is enough headroom for a normal run. If a partial run does show up, look at Sentry (production errors go there, not Vercel logs) before assuming a timeout. Signature of a partial run: batch row count well below the recent daily range (as of 2026-09-07, cumulative batch+batch_continuation daily totals run ~7,800–7,900 rows/day; confirm the current range against the last several full runs).
- **`batch_continuation` is a same-run self-continuation, not a manual catch-up or a separate scheduled surface.** When the scheduled 07:00 UTC run hits its ~240s per-invocation budget mid-catalog, `server/cron.pricing-batch-recompute.ts` fires a follow-up POST with `trigger: 'batch_continuation'` to finish the day's recompute, writing a second cluster of `pricing_audit_log` rows minutes after the first. Judge daily completeness from the **cumulative day total across `batch` + `batch_continuation` rows** (or read the cron handler's own `dayTotal` / `isPartialBatchDay` verdict, exported from `recomputeCatalog`/`getRecentBatchDayCounts`), never from the `batch`-only row count in isolation — a healthy split-batch day can show a `batch`-only slice well under the old single-cluster baseline while the cumulative total is normal.
...
2. **If yes — was it complete?** Compare the **cumulative `batch` + `batch_continuation` row count** for the day to the expected catalog size (as of 2026-09-07, full days log ~7,800–7,900 rows/day cumulative; confirm current size against the most recent full days rather than hardcoding). A `batch`-only slice below that range is not on its own a partial run — check whether a same-day `batch_continuation` cluster completes it first. Only a cumulative total far below the range means the function was killed mid-run (see maxDuration risk). Report a genuinely partial day as a failure, not a success.
