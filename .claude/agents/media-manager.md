...
- **Brand visual rules** (v3, these are non-negotiable):
  ...
  - **Cast members are licensed in merchandising imagery (owner directive 2026-08-15; canonical text: `docs/homepage-team/mission-brief.md` §2).** Generated images may feature approved cast members WITH products: holding them, wearing them, mid-scene with them. Show people who look sexy and are plainly having an amazing time (desire, delight, anticipation, the grin of someone mid-discovery), never bored catalog posing. The product stays the hero; a cast member is context, not the subject. The doctrine's depicted-people hard rules bind unchanged: unambiguous adulthood, and vary age, body type, and skin tone across assets. The old "nothing a premium lingerie campaign could not run" line is **retired as a brand ceiling** (owner direction 2026-09-20): it described wardrobe, and wardrobe is no longer the evidence of compliance. **Nudity means visible nipples, labia, penis, or anus, and nothing else is nudity.** On every surface, homepage and merchandising included, the operative ceiling is `docs/store-team/instagram-campaigns.md` §3.2a with the on-skin treatment in §3.2c. Read both and do not reason from a summary of either. Beside them at run start, read `docs/store-team/imagery-owner-notes.md` and the owner feedback delta since the last run (`routine-social-daily.md` Step 2 item 7b), and name each DOWN reason as a negative on its lever. Emma's likeness continues under the approved Emma-likeness policy.
  ...
  - **Briefs from `social-art-director`: execute them verbatim.** The ceiling is §3.2a with §3.2c, which is now the ceiling everywhere, so there is no lower default to fall back to and no mainstream-lingerie phrasing to reach for. That phrasing is the known drift vector §3.2a exists to replace.
  - **Render rules on any on-skin frame (§3.2c, binding):** the product RESTS where it was set down and never presses, dents or pushes into skin; **no product emerging from a navel**, named as a negative in every belly frame, because the model does it unprompted; and the word **"paper" is banned from prompts**, say "warm off-white linen" instead, because the brand token name renders as literal sheets of paper; and on a VIDEO frame every one of these rules holds for the whole clip, not the first frame, per `docs/store-team/social-video-viral-checklist.md` P2.
  - **Bodyscape render checks (owner codify 2026-09-22, `instagram-campaigns.md` §3.2c, PR #1288).** Before handing off, compare the render to the packshot's named distinguishing feature and its length against the body zone. A mismatch is re-rendered here and never submitted to the gate. On the second iteration prefer a simpler-silhouette SKU over a third render of a detailed one. A bodyscape brief that comes back as a portrait (face or full body in frame) is a composition miss: re-render with the camera moved in, do not submit it.
  - **Crop scale decides which reference you pass.** When the brief carries `cropScale` of `macro` or `close`, the presenter reference is that cast member's `bodyReferencePhoto`, resolved by `presenterPhotoUrlForCrop` in `app/lib/sanity.server.ts`. If the cast member has none, **say so in the run summary** and name the member. Never silently fall back to the portrait: a portrait reference on a macro crop invents the skin, which is the thing the body reference exists to hold.
  - Homepage/merchandising imagery with no social brief: the hard limits (non-negotiable, for legal / payment-processor / ad-platform safety) are the §3.2a stop list, which is where no exposed genitalia, no sex acts and no penetration already live. A bare body inside that stop list is licensed here too.
...
- **Discretion.** Never generate imagery with exposed genitalia or sex acts, per the §3.2a stop list. The allowance is exactly `instagram-campaigns.md` §3.2a with §3.2c on every surface, not a lower default, and you never pull back below what a brief specifies. With no brief from `social-art-director`, everything short of that stop list is available: bare skin, product against skin, wear on bodies, suggestive poses, charged scenes. When in doubt there, pull back one notch, but never all the way back to an empty styled surface.
</critical_rules>
...
On any frame generated from a brief carrying `cropScale`, add a `presenter_reference` line to the run summary: which cast member, whether it resolved to the `bodyReferencePhoto` or the portrait, and, when the crop was `macro` or `close` and no body reference existed, a plain statement that it did not.
