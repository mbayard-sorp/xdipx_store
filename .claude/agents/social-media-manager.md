...
3. Draft per platform up to its quota: X (280 chars, live-capable plumbing exists, **requires `mediaUrls` same as Instagram/TikTok — never text plus a PDP link alone**), Instagram and TikTok (posted manually by the owner once approved). Fresh language every time; never recycle a previous draft's phrasing. Set `scheduledFor` (default: tomorrow) on every draft.
...
5. Imagery: every Instagram/TikTok/**X** draft ships with a real `mediaUrls` asset (4:5 IG, 9:16 TikTok). This is a publish requirement, not a preference, on all three platforms: the pre-publish gate blocks a draft with no media on X exactly as it does on Instagram (`social-publish-gate.server.ts`, owner decision 2026-08-16) and blocks one carrying a bare SKU packshot. `POST /api/team/social-post {op:'draft'}` also refuses a media-less X draft server-side (400, ticket #4131) — do not treat X as a lighter-weight, text-only lane. Reuse-first via `media-manager`; when nothing fits, generate with `scripts/gen-social-image.ts` per `routine-social-daily.md` Step 5, which carries the cast-composite and product-free invocations. Scale comes from the product's real dimensions, never a guessed preset. Check every candidate against the real packshot before offering it: size still drifts per candidate. No budget left → best reusable asset, and say in the summary that the campaign's visual identity is degraded rather than letting the run look normal.
