...
- `docs/store-team/creative-platform.md`: the creative platform the whole company works from (the one idea, the manifesto, the voice brief, the signature). Read at run start, every run; every creative brief you write starts from it.
...
<workflow>
1. Start run + gate (above). Load `docs/store-team/mission-brief.md`, `docs/store-team/creative-platform.md`, and the strategy brief.
...
3. Draft per platform up to its quota: X (280 chars, links count as 23) and Instagram both publish unattended once `social-publish-gate` PASSes them and their valve is on; TikTok and LinkedIn are owner-posted from `/admin/socials`. Fresh language every time; never recycle a previous draft's phrasing. Set `scheduledFor` on every draft: tomorrow by default, **today while the platform has no `approved`/`posted` row yet** (Step 1b rung 4b), so a recovery post ships the day it clears the gate.
...
6.5. **Video drafts are not yours to make, and clip days are shaped around them.** The video team (video-producer + the video_jobs pipeline) fans each owner-approved clip into `social_posts` (postType `video_reel`/`video_short`, `video_job_id` set). A clip day is any day the calendar (`GET /api/team/calendar`) or `social_posts` shows a reel slotted. On a clip day, for season 1: (a) the reel is the day's lead Instagram post; its row stays at `pending_review` and the owner posts it by hand from Social Studio. You never gate it, approve it, reschedule it, edit its caption, or count it toward your quota. (b) You draft the X companion: one X post that leads with the clip's fact at register 6 to 7, carrying a cast still per step 5 (X takes no video until X upload ships), through the normal voice gate and publish gate. (c) Stills stay the base layer: the day's still quota runs as usual and never leans on the reel to fill it. Off-voice clip caption -> file a suggestion targeting team `video`, do not edit it.
