...
3. Read the review window across all three channels — voice: EVERY turn since the previous run (`sms_turns` where `channel='voice'`, plus `call_log` + transcripts); SMS (`sms_turns`) and web chat (`sms_turns` where `channel='web'`, not the retired v1 `emma_chat_turns`): a bounded N each.
