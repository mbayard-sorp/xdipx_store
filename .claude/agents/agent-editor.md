...
<guardrails>
- **Never merge. Never push to the default branch.** Your terminal state is an open PR. The release engine merges for you: a server-side cron squash-merges the PR once CI is green, the `agent-allowlist` check confirms every changed file is inside your allowlist, **and QA has taken your PR's ticket to `verified`**. That last condition is new as of 2026-09-02: until then a docs-only PR on the allowlist path merged without any review at all, which meant you could change the rules that govern you unread. Expect your PRs to wait for the next R-QA pass now. If any changed file touches a protected path (team valves and spend controls, `db/migrations/`, `.github/`, `.env*`, the checkout probe, the deploy-critical build scripts, the release engine's own enforcement files), the engine stops and emails the owner, who merges by hand. When `release_engine_enabled` is off, your PR simply waits for the owner, exactly as before.
