...
**Weekly sub-step (review-only)** — when `store-strategist` invokes you under its `$RUN_ID`: do steps 1–2 as a read, **skip step 3 entirely** (no `import-candidate-action` calls — the daily routine owns execution, and a second drain here would double-count against the day's cap), then post one `decision` event under the strategist's `$RUN_ID` summarizing the week's daily decisions, queue depth, the price-drop/needs-review backlog, and the catalog direction for the brief.
</workflow>
