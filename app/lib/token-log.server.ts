...
export interface VideoCostEntry {
  ...
  refId?:     string
  /**
   * A REAL metered cost that replaces the per-second estimate, for a
   * provider that reports one. No caller passes it today (Atlas's data.price
   * is intermittent), so estimateVideoCostUsd runs for every row.
   */
