...
- orderStatus looks up an existing order (shipping/tracking/refund/cancel). Call it whenever the customer asks about an order they already placed ("where's my order?", "has it shipped?", "tracking", "can I cancel/refund #1042?"). It uses their phone automatically, so never ask for a phone. Answer from the tool result; if it returns order_not_found or no_caller_phone, follow that result's guidance (ask for the order number, or point them to hello@xdipx.com). Do not invent a status.
- lookupPolicy answers shipping, returns/refund, compatibility, troubleshooting, and general brand-FAQ questions from the knowledge base. Call it for policy and how-it-works questions ("what's your return policy?", "how long is shipping?", "is this waterproof-safe with silicone lube?", "how do I charge it?"). Answer from the tool's quickAnswer; if it returns not_found, don't guess a policy, offer to connect them with the team at hello@xdipx.com.
...
- If they ask about delivery / shipping / order status: call orderStatus (it uses their phone automatically) and answer from the tool result. Only if the tool returns no_caller_phone or order_not_found, say plainly "I don't have visibility into that from here. Email hello@xdipx.com or call (623) 900-1188 and the team will sort it." Don't promise a callback. For a shipping-policy question rather than a specific order, use lookupPolicy.
...
export async function executeConversationAgent(
  ...
  try {
    for (let hop = 0; hop < MAX_TOOL_HOPS; hop++) {
      ...
      // rather than start a call that could blow Twilio's webhook window. No-op
      // outside a deadline scope (getTurnRemainingMs is Infinity).
