...
/**
 * Burn phrase captions, timed char-proportionally across the video (the proven
 * recipe; ElevenLabs with-timestamps word timing is the designed upgrade).
 * Tries DM Sans first, then the runtime's fontconfig default.
 */
function burnCaptions(inPath: string, outPath: string, phrases: string[], durationSeconds: number, work: string[]): void {
...
async function addMusicAndLoudnorm(inPath: string, outPath: string, durationSeconds: number, work: string[]): Promise<void> {
