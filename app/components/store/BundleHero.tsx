  ...
  const content = (
    <div className={compact ? 'space-y-4' : 'grid md:grid-cols-2 gap-8 lg:gap-12 items-start'}>
      <div className={compact ? '' : 'order-2 md:order-1'}>
        <p className="text-brand-purple text-sm font-semibold uppercase tracking-widest">
          ♥ Bundle Deal
        </p>
        <h1
          className={
            compact
              ? 'text-xl font-bold text-brand-charcoal mt-1 leading-snug'
              : 'text-2xl md:text-3xl font-bold text-brand-charcoal mt-1 leading-snug'
          }
        ...
        {bundle.tagline && (
          <p className="text-brand-charcoal/70 mt-2 italic">{bundle.tagline}</p>
        )}
        ...
        <div className="flex items-baseline gap-3 flex-wrap mt-4">
          <span
            className={
              compact
                ? 'text-2xl font-black text-brand-gradient'
                : 'text-4xl font-black text-brand-gradient'
            }
            style={{ fontFamily: 'var(--font-display)' }}
          >
        ...
        {savings > 0 && (
          <p className="text-sm text-brand-purple font-semibold mt-1">
          ...
          {bundle.components.map(c => {
            ...
            return (
              <li key={c.product.handle} className="flex items-center gap-3">
                <div className="w-14 h-14 rounded-lg overflow-hidden bg-brand-mist shrink-0">
                  {img ? (
                    <img
                      src={img.url}
                    ...
                    <div className="w-full h-full flex items-center justify-center text-brand-charcoal/20 text-xl">♥</div>
                  )}
                </div>
                <div className="flex-1 min-w-0">
                  <Link
                    to={`/products/${c.product.handle}`}
                    className="text-sm font-semibold text-brand-charcoal hover:text-brand-coral transition-colors line-clamp-2"
                  >
                    {c.product.title}
                  </Link>
                  {c.quantity > 1 && (
                    <span className="text-xs text-brand-charcoal/50 ml-1">× {c.quantity}</span>
                  )}
                </div>
                <span className="text-sm text-brand-charcoal/60 tabular-nums">
      ...
      {!compact && (
        <div className="order-1 md:order-2 grid grid-cols-2 gap-2">
          {bundle.images.slice(0, 4).map((img, i) => (
            <div
              key={i}
              className={`rounded-2xl overflow-hidden bg-brand-mist ${
