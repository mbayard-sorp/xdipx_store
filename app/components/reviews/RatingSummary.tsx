...
export function RatingSummary({ aggregate, productId }: RatingSummaryProps) {
  ...
  return (
    <section className="bg-white rounded-2xl border border-cream-2 p-6 mb-6">
      <div className="flex flex-col sm:flex-row gap-6 items-start">
        ...
        <div className="flex flex-col items-center sm:items-start shrink-0">
          ...
          <span className="text-sm text-ink/50 mt-1">
        ...
        <div className="flex-1 w-full space-y-1.5">
          {bars.map(({ stars, count }) => (
            <div key={stars} className="flex items-center gap-2">
              <span className="text-xs text-ink/60 w-4 text-right shrink-0">{stars}</span>
              ...
              <span className="text-xs text-ink/50 w-6 shrink-0">{count}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Badges */}
      {approvedCount > 0 && (
        <div className="flex flex-wrap gap-3 mt-5 pt-4 border-t border-cream-2">
          ...
          {verifiedCount > 0 && (
            <span className="inline-flex items-center gap-1.5 bg-cream-2 text-sage text-xs font-medium px-3 py-1 rounded-full">
              <span aria-hidden="true">✓</span>
              {verifiedCount} verified purchase{verifiedCount !== 1 ? 's' : ''}
            </span>
          )}
          {withPhotoCount > 0 && (
            <span className="inline-flex items-center gap-1.5 bg-cream-2 text-ink/60 text-xs font-medium px-3 py-1 rounded-full">
              <span aria-hidden="true">📷</span>
      ...
      <div className="mt-5">
        <Link
          ...
          className="inline-block bg-coral text-white font-semibold text-sm px-6 py-2.5 rounded-full hover:opacity-90 transition-opacity"
          style={{ fontFamily: 'var(--font-display)' }}
        >
          Write a Review ♥
        </Link>
