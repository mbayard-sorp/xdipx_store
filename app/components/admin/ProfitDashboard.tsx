    ...
    <div>
      ...
      <div className="bg-white rounded-2xl overflow-hidden shadow-sm">
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.map(row => (
              <tr key={row.summaryDate} className="border-t border-cream-2 hover:bg-cream-2/50 transition-colors">
                <td className="px-4 py-3 font-medium">{row.summaryDate}</td>
                <td className="px-4 py-3 text-ink/60">{row.featuredSku ?? '—'}</td>
                ...
                <td className={`px-4 py-3 text-right font-semibold ${parseFloat(row.totalProfit ?? '0') > 0 ? 'text-green-600' : 'text-ink/40'}`}>
...
function KpiCard({ label, value, highlight = false }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className={`rounded-2xl p-5 ${highlight ? 'bg-coral text-white' : 'bg-white shadow-sm'}`}>
      <p className={`text-xs font-medium mb-1 ${highlight ? 'text-white/70' : 'text-ink/50'}`}>{label}</p>
      <p
        className={`text-2xl font-black ${highlight ? 'text-white' : 'text-ink'}`}
