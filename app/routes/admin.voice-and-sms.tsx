...
export const meta: MetaFunction = () => [{ title: 'Voice & SMS — xdipx Admin' }]
...
const DEFAULTS: Record<string, string> = {
  ...
  ...IVR_CONFIG_DEFAULTS,
  // SMS template overrides — resolved at runtime by app/lib/sms-v2/sms-config.server.ts.
  // Empty default makes the textarea show as blank (placeholder-ready) so admins
  // see "no override saved → using compiled-in copy" rather than the default
  // text pre-filled (which would make every save look like a no-op edit).
  smsAgeWelcome: SMS_DEFAULTS.ageWelcome,
  smsReturningGreeting: SMS_DEFAULTS.returningGreeting,
      ...
      <section className="bg-white rounded-2xl p-6 shadow-sm space-y-6">
        <div className="space-y-2">
          ...
          <p className="text-xs text-ink/60 leading-relaxed">
            Three editable strings that drive the SMS thread at three specific moments.
            <strong> Age confirmed</strong> is the first thing a new shopper sees right after replying YES to the 18+ disclosure —
            it should land warm and inviting and tee up Emma. <strong>Returning greeting</strong> is the prefix used when a
            customer with order history texts in (use the <code>{'{firstName}'}</code> slot to address them by name when we know it).
            <strong> Checkout closing</strong> is the closer that ships the cart link — keep <code>{'{url}'}</code> on its own line
            so the iMessage preview shows the URL cleanly.
          </p>
          <p className="text-xs text-ink/50 italic">
            The 18+ disclosure itself (the prompt that asks &ldquo;reply YES to confirm&rdquo;) is regulatory copy and is
            intentionally NOT editable from this page. If it has to change for legal reasons, edit{' '}
            <code>AGE_GATE_REPLY</code> in <code>app/lib/sms-processor.server.ts</code> directly.
          </p>
        </div>
        ...
        <SaveForm
          fetcher={fetcher}
          settings={settings}
          label="Age-confirmed welcome"
          settingKey="smsAgeWelcome"
          multiline
          rows={3}
          description='Sent right after the customer replies YES to the 18+ gate. Default: "You&apos;re in. I&apos;m Emma — ask me anything about our daily deals, products, or your orders. What&apos;s on your mind?"'
        />
        ...
        <SaveForm
          ...
          settingKey="smsReturningGreeting"
          multiline
          rows={2}
          description="Prefix Emma drops in when a customer with prior history texts in. Use {firstName} where their name should land. Leave empty if you'd rather skip the welcome-back beat entirely."
        />

        <SaveForm
          fetcher={fetcher}
          settings={settings}
          label="Checkout closing"
          settingKey="smsCheckoutClosing"
          multiline
          rows={3}
          description="The closing line that ships the cart link. Use {url} where the Shopify checkout URL should appear — substitution happens at send time. Recommended: put {url} on its own line so the iMessage rich preview renders correctly."
        />
      </section>
    </div>
  )
