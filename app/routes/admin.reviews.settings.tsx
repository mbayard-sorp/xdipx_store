    ...
    <div className="flex items-start justify-between gap-4 py-3 border-b border-cream-2 last:border-0">
      ...
      <div className="flex gap-3 shrink-0">
        <label className="inline-flex items-center gap-1.5 text-xs text-ink/60">
          <input type="radio" name={name} value="true"  defaultChecked={defaultChecked}  className="accent-sage" />
          On
        </label>
        <label className="inline-flex items-center gap-1.5 text-xs text-ink/60">
          <input type="radio" name={name} value="false" defaultChecked={!defaultChecked} className="accent-sage" />
          Off
        </label>
      </div>
    </div>
  ...
  return (
    <div className="max-w-2xl">
      ...
      <Form method="post" className="space-y-6">
        {/* 1. Moderation */}
        <section className="bg-white rounded-2xl border border-cream-2 p-5">
          <p className="font-semibold text-ink mb-4 text-sm" style={{ fontFamily: 'var(--font-display)' }}>
            Moderation
          </p>
          ...
          <div className="py-3 border-b border-cream-2">
            <label className="text-sm font-medium text-ink block mb-1">Spam threshold</label>
            <p className="text-xs text-ink/50 mb-2">Reviews scoring above this are auto-flagged as spam (0–1)</p>
            <input
              name="spamThreshold"
              type="number"
              min="0" max="1" step="0.05"
              defaultValue={settings.spamThreshold}
              className="border border-cream-2 rounded-xl px-3 py-2 text-sm w-32 focus:outline-none focus:border-sage"
            />
          </div>
          <div className="py-3">
            <label className="text-sm font-medium text-ink block mb-1">Min body length (chars)</label>
            <input
              name="minBodyLength"
              type="number"
              min="0" step="1"
        ...
        <section className="bg-white rounded-2xl border border-cream-2 p-5">
          ...
          <div className="py-3 border-b border-cream-2">
            <label className="text-sm font-medium text-ink block mb-1">Invite delay (days after fulfillment)</label>
            <input
              name="inviteDelayDays"
              type="number"
              min="1" step="1"
              defaultValue={settings.inviteDelayDays}
              className="border border-cream-2 rounded-xl px-3 py-2 text-sm w-24 focus:outline-none focus:border-sage"
            />
          </div>
          <Toggle name="remindersEnabled" label="Send reminder emails" defaultChecked={settings.remindersEnabled} />
          <div className="py-3">
            <label className="text-sm font-medium text-ink block mb-1">Reminder delay (days after invite)</label>
            <input
        ...
        <section className="bg-white rounded-2xl border border-cream-2 p-5">
          ...
          <div className="py-3 border-b border-cream-2">
            <label className="text-sm font-medium text-ink block mb-1">Reviews per page</label>
            <input
              name="reviewsPerPage"
              type="number"
              min="5" max="50" step="5"
          ...
          <div className="py-3 border-b border-cream-2">
            <label className="text-sm font-medium text-ink block mb-1">Default sort</label>
            <select
              name="defaultSort"
              defaultValue={settings.defaultSort}
              className="border border-cream-2 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-sage"
            >
        ...
        <section className="bg-white rounded-2xl border border-cream-2 p-5">
          ...
          <div className="py-3 border-b border-cream-2">
            <label className="text-sm font-medium text-ink block mb-1">Digest email</label>
            <p className="text-xs text-ink/50 mb-2">Receive daily new review digest</p>
            <input
              name="digestEmail"
              type="email"
              defaultValue={settings.digestEmail ?? ''}
              placeholder="admin@xdipx.com"
              className="border border-cream-2 rounded-xl px-3 py-2 text-sm w-full max-w-sm focus:outline-none focus:border-sage"
            />
          </div>
          <div className="py-3">
            <label className="text-sm font-medium text-ink block mb-1">Webhook URL</label>
            <p className="text-xs text-ink/50 mb-2">POST on each new review (optional)</p>
            <input
              name="webhookUrl"
              type="url"
              defaultValue={settings.webhookUrl ?? ''}
              placeholder="https://..."
              className="border border-cream-2 rounded-xl px-3 py-2 text-sm w-full focus:outline-none focus:border-sage"
            />
          </div>
        </section>
        ...
        <button
          type="submit"
          className="bg-coral text-white font-semibold px-8 py-3 rounded-full hover:opacity-90 transition-opacity"
          style={{ fontFamily: 'var(--font-display)' }}
        >
          Save Settings ♥
        </button>
