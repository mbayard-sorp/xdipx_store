...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    switch (intent) {
      ...
      case 'retry-frames': {
        // sceneIndex present = multi-scene retry (ticket #5714 added the
        // pipeline branch; this finally un-hides the input for those jobs).
        const rawSceneIndex = form.get('sceneIndex')
        const sceneIndex = rawSceneIndex != null && String(rawSceneIndex) !== '' ? Number(rawSceneIndex) : undefined
        await retrySceneFrames(jobRowId, String(form.get('feedback') ?? ''), sceneIndex)
        return Response.json({ ok: true })
      }
      case 'reject': {
              ...
              <div className="mt-3 flex flex-col gap-3 md:flex-row">
                {(
                  <Form method="post" className="flex flex-1 gap-2">
                    ...
                    <input type="hidden" name="jobRowId" value={job.id} />
                    {/* Multi-scene retry (tickets #5714/#5716): target the scene
                        currently parked for approval. Single-scene jobs send no
                        sceneIndex and take the legacy path. */}
                    {job.scenes && (
                      <input
                        type="hidden"
                        name="sceneIndex"
                        value={(job.scenes ?? []).findIndex((sc, i) => sc.continuity !== 'last-frame' && (job.sceneState ?? [])[i]?.status === 'awaiting_frame_approval')}
                      />
                    )}
                    <input
                      name="feedback"
