...
export const meta: MetaFunction = () => [{ title: 'PM Chat — xdipx Admin' }]

export async function loader({ request }: LoaderFunctionArgs) {
  await requireAdmin(request)
  const { thread, messages } = await getOrCreateCurrentThread('pm')
...
function usePmStream(threadId: number, onComplete: () => void) {
  ...
  useEffect(() => {
    ...
    const es = new EventSource(`/api/admin/pm-chat/stream/${threadId}`)
    esRef.current = es
...
export default function PmChat() {
  ...
  const { streamingDraft, streamingTools, isStreaming, streamError, kick } = usePmStream(
    thread.id,
    revalidate,
  )

  // Auto-kick when the page loads with a pending user message and no
  // assistant reply yet (same pattern as Emma chat).
      ...
      <div className="flex items-center justify-between gap-3 mb-4 shrink-0">
        <div className="min-w-0">
          ...
          <p className="text-xs text-muted mt-0.5" style={{ fontFamily: 'var(--font-body)' }}>
            Catalog and merchandising intelligence. Proposes only — humans approve.
          </p>
        </div>
  ...
  return (
    <details className="mb-1">
      ...
      <div className="mt-1 ml-4 p-3 bg-cream rounded-lg">
        <pre
          className="text-xs text-ink/70 overflow-x-auto whitespace-pre-wrap break-words"
          style={{ fontFamily: 'var(--font-body)' }}
        >
          {event.content ?? '...'}
        </pre>
