...
| p4-events | Per-section GA4 engagement events + per-module PDP click-through | 4 | rr7-engineer (PR) | 2026-08-03 | in-progress | AMBER | section-visibility events wired in the analytics layer on main | 2026-08-03 | gives the 70% product-link rule an outcome metric. Audit 2026-08-03: `home_variant_view`, `home_scroll_depth` (section param), and `cta_click` (with `cta_location`) confirmed in `analytics.client.ts`; per-module PDP click-through still not confirmed. Target week (W4) now reached with partial evidence → AMBER. |
| p4-changelog | Design changelog appended by both routines | 4 | homepage-orchestrator | 2026-08-03 | not-started | AMBER | `docs/homepage-team/design-changelog.md` exists and has post-launch entries | 2026-08-03 | Audit 2026-08-03: file still does not exist. Target week (W4) reached with zero evidence → AMBER; flips RED next Monday if still absent. Cheap to close (a doc-append by homepage-orchestrator). |
| p4-retro | First measured design retro + critic-score dashboard panel | 4 | homepage-orchestrator + rr7-engineer | 2026-08-03 | not-started | AMBER | dashboard shows critic scores AND a retro event references them | 2026-08-03 | definition of done: critic avg ≥4.5 four straight weeks. Audit 2026-08-03: target week reached; the critic-score series has only ~1-2 points (last 4.07), so the four-straight-weeks bar is structurally unreachable this week → AMBER until the series exists. |
