...
| p0-3-enrich | Enrichment batch drained: draft cohort + mood/audience/matters tag coverage | P0 | emma-product-enricher + media-manager | 2026-07-27 | in-progress | AMBER | `import_candidates.enriched_at`/`published_at` show a healthy live-draining cohort (this run's probe; supersedes the retired `batch_jobs` mechanism) AND `mood_tags` coverage on non-archived products approaches saturation | 2026-08-10 | gates p2-1-pseo; improves discovery rails + PLP facets. RE-AUDIT 2026-08-10: the RED-on-`batch_jobs`-is-dead premise carried for ~3 briefs does not hold anymore (flagged by product-team suggestions #1623/#1784/#2156) — the enrichment mechanism changed (cheaper single-call enrichment replaced the batch path; `batch_jobs` now sits empty/unused, the original probe was written against a retired mechanism). Real evidence: max enriched/published both 2026-08-10, 108 enriched + 108 published in the last 7 days, only 24 stuck (down from 289 stuck for weeks), 0 stuck on the publish half. Catalog-wide: tagline 99.1%, FAQs 98.9%, sensation dial 98.9%, but `moodTags` only 49.3% — a real, already-tracked gap (approved suggestion #1278). RED→AMBER: the dead-chain half is resolved, the tag-coverage half is not. Do not carry the stale dead verdict forward. Audit 2026-08-24: Shopify-blind this session (no Shopify creds in sandbox), so the `moodTags` catalog-coverage half is unverified again this run — capped at AMBER per README, not guessed. The live-draining-cohort half stays healthy (enriched_7d 79, published_7d 79 per this run's evidence), but `enrich_stuck` doubled week over week (26→52) and `enrich_failed`(parked) nearly tripled (~10→28) — a real, growing risk worth a closer look even though it hasn't broken the tracked AND-condition yet. AMBER, unchanged. Audit 2026-08-31: Shopify-blind again this session (confirmed via a direct `.env*` check: only `.env.example` present, no real credentials), so `moodTags` coverage stays unverified, capped at AMBER. The live-draining-cohort half stays healthy on this week's officially captured numbers (enriched_7d 130, published_7d 130). `enrich_stuck` ticked up again (52→53) and quality-gate-parked held flat at 28 — the same growing-but-not-accelerating risk flagged the last two audits, still not worth a suggestion on its own. AMBER, unchanged. Audit 2026-09-07: Shopify-blind again this session (only `.env.example` present, confirmed directly), `moodTags` coverage stays unverified, capped at AMBER. Real, fresh evidence on the live-draining-cohort half, and it's a genuine active break, not a slow drift: enriched/published last 7d both healthy at 89, but max-enriched and max-published timestamps are BOTH stuck at 2026-09-05 despite the enrich cron reporting `succeeded` on 09-06 and 09-07 — the cohort has stopped draining for two straight days while the routine reports healthy. Root cause is already identified and tracked: ticket #7993 (approved, no PR) — the live Nalpac feed's Brand column has been empty for all 16,573 rows since ~09-05, so `collapseMasters()` returns zero masters catalog-wide and `approveAndImport` fails "master no longer in feed" for every candidate, taking catalog throughput to zero on 09-06 and 09-07. `enrich_stuck` is actually down (53→29, real improvement) and `enrich_failed`(parked) held flat at 28, both measured before this feed defect's effect fully lands in the numbers. AMBER, unchanged — already covered by #7993, non-terminal, not refiled. Audit 2026-09-14: the two-day stall is over. #7993 (Nalpac feed Brand-column outage) cleared 2026-09-11 per product-manager's cross-check this run; live-draining-cohort resumed same-day-fresh (`max(enriched_at)`=today 2026-09-14, `max(published_at)`=2026-09-13, both within the normal same-day/one-day lag this tracker has always treated as healthy), enriched_7d 93, published_7d 69, and `enrich_stuck` fell sharply to 7 (from 29 last audit). `moodTags` catalog coverage not independently re-run this session (Shopify Admin token was available this run but a full-catalog metafield sweep was out of this audit's time budget) — carried at last measured ~49.3%, still under approved #1278, stays UNMEASURED rather than guessed, caps this at AMBER. **Audit 2026-09-22 (program-manager, run 1021): AMBER, unchanged, live-draining-cohort stays healthy.** `enriched_at`/`published_at` both max out today (2026-09-22), enriched_7d and published_7d both 138 — genuinely current, not stale. `enrich_stuck` (imported, never enriched) holds at 7 (matches last audit's "fell sharply to 7", now confirmed stable rather than a one-off dip); quality-gate-parked flat at 28. Shopify-blind again this session (only `.env.example` present, confirmed directly), so `moodTags` catalog coverage cannot be independently re-run — stays UNMEASURED, caps at AMBER per this tracker's own rule. **Flag, not yet acted on**: owner blocker #119's research (2026-09-20, p0-7-valves above) claims near-saturation on a 750-product sample (`mood_tags` 0 misses, `audience_tags` 0 misses, `matters_tags` 1 miss) — if true this would flip this milestone AMBER->done, but it is another agent's secondary research, not a probe run by this audit, so it is not acted on here. Next session with Shopify Admin access: pull the live number from `admin.imports.tsx`'s `CoveragePanel` (the milestone's own already-shipped dashboard, p1-8-pdp) rather than re-sampling by hand.
| p0-4-reviews | Review corpus live: invite flow verified, real approved reviews, `reviews_pdp_enabled` on | P0 | shopify-ops + owner | 2026-07-27 | not-started | RED | `pipeline_settings.reviews_pdp_enabled='true'` AND ≥1 approved review row AND aggregateRating present in a live PDP page source | 2026-09-07 | valve must not flip before real reviews exist (055 comment / Google policy). Audit 2026-07-27: target week reached, `reviews_pdp_enabled=false` (correctly off), `review_invites` still 0 rows ever sent, raw `reviews` ~1 row. No movement at target. AMBER→RED. Suggestion #114 filed. Audit 2026-08-31: still 0 review invites ever (`review_invites` re-confirmed empty), `reviews_pdp_enabled` correctly `false`. New context this run: Shopify confirms the store's first-ever non-owner order, #1003 (2026-08-20, $28.11, "Rayette Zerbel") — 3 lifetime orders, 1 real customer, and zero invites sent against any of them. The root blocker (the review-invite pipeline itself not firing) is already covered by suggestion #5233 (`review-invite-pipeline-not-firing-2026-08`, blocked, non-terminal, targets strategy); not refiled. RED, unchanged. Audit 2026-09-07: `review_invites` confirmed still 0 rows ever. **The store's second real, non-owner order landed this week** (#1004, 2026-09-07, Shawn Volk, $19.99) — 4 lifetime orders, 2 real customers, still zero invites sent against either. Still covered by suggestion #5233, non-terminal, not refiled. RED, unchanged. Audit 2026-09-14: `review_invites` re-confirmed 0 rows ever (direct query). Shopify Admin API (order list, no PII fields) confirms still exactly 4 lifetime orders, most recent still #1004 (09-07) — no new order this week, so the invite gap has not grown, but it also has not moved. Owner blocker #127 (confirm order #1003/#1004 customer email for review invites, or confirm the pipeline itself) remains open. Still covered by suggestion #5233, non-terminal, not refiled. RED, unchanged. **Audit 2026-09-22 (program-manager, run 1021): RED, unchanged, but the ask is now bounded and the old suggestion is stale.** `review_invites` re-confirmed 0 rows ever (direct query). Owner blocker #127 (researched 2026-09-20) narrowed the question to exactly one remaining task: the webhook half is ANSWERED — PR #1190 / ticket #9329 (confirmed live, `server/webhooks.ts:388`) stamps the logged-in shopper's email onto the cart and recovers it in `handleOrderFulfilled`'s fallback chain since 2026-09-15, so future account-holder orders create invites on their own. Guest-checkout orders are still blind (Shopify Basic-plan PII redaction on the order/customer payload, confirmed by a direct Admin API probe this session returning null email fields and GraphQL ACCESS_DENIED on the same order). The 4 lifetime real-customer orders (now includes the third, 2026-09-17 $81.98) all predate the fix and cannot be recovered from any API — the only path is the owner pasting each order's email (visible to him in the Shopify Admin UI) into `emails.json` and running `scripts/backfill-review-invites.ts --emails-file emails.json --apply` (dry-run by default, idempotent). **Suggestion #5233 (`review-invite-pipeline-not-firing-2026-08`, blocked) is now STALE**: it asks whether the webhook/cron is broken, which blocker #127 already answered (it is not, for account-holder orders going forward) — the strategist should comment on #5233 with this evidence rather than leave it open chasing a resolved question; not re-filing. Also corrected: this tracker's own reviews-precondition read must not treat the single `reviews` row as real — it is a fabricated test record (`ip_address:'unknown'`, `ai_spam_score:0.70`, see p0-7-valves L6 above) and `reviews_pdp_enabled` must stay off until the backfill produces a real one. RED, unchanged.
...
| p0-7-valves | §3 live-state checklist resolved (052 state, GSC env, SEO valves, stock indicator) | P0 | owner + shopify-ops | 2026-07-20 | in-progress | AMBER | checklist L1–L9 answers recorded in this tracker's status log AND stock indicator un-commented on the PDP | 2026-09-22 | answers may re-scope other milestones. Audit: L1–L9 never recorded in this log; PDP stock indicator still commented out (`app/routes/_layout.products.$slug.tsx` L61, L867-868). Suggestion #58 filed. Audit 2026-08-31: L1–L9 checklist still never recorded in this log, now 6 weeks past the 2026-07-20 target. PDP stock-indicator half remains the only part of the probe that passes. RED, unchanged — cheapest unblock in the roadmap, unmoved for 6 straight audits. Audit 2026-09-07: still never recorded in this log, now 7 weeks past target. Owner blocker #119 (filed 09-06, open) now names this exact checklist directly by tracker id — the ask has moved from this tracker's own carried note onto the owner's blocker list, which is the more visible surface. RED, unchanged. Audit 2026-09-14: blocker #119 confirmed still open, L1-L9 still never recorded in this log, now 8 weeks past target. RED, unchanged. **Audit 2026-09-22 (program-manager, run 1021): RED -> AMBER, real movement.** Owner blocker #119 (researched 2026-09-20, still technically `open` pending the owner's own confirm-and-paste) did the checklist work directly and this audit independently corroborated the parts a Neon query can reach: L1 (migration 052) — `pipeline_settings` valve rows cluster at 2026-07-09/07-16/08-04, consistent with staged rollout, not one big-bang date, but no contradiction found. L4 (GSC env) — `gsc_snapshots` holds 10 rows now (was 9 on 09-20), confirming the Monday cadence is live. L3/L5 (cron/trigger firing) — 19 distinct routes wrote `cron_runs` rows in the trailing 14 days (blocker #119 measured 18 on 09-20), consistent; trigger existence in Claude's scheduler itself remains unreadable from this session, so L5 stays PARTIAL, as blocker #119 already flagged. L6 (reviews precondition) — corrected by blocker #119 to UNMET: the one `reviews` row is a fabricated test record (`ip_address:'unknown'`, `ai_spam_score:0.70`), not real proof; this tracker will not treat it as satisfying anything (see p0-4-reviews below). L7 (enrichment coverage) and L8 (Klaviyo greenfield) are blocker #119's own research, not independently re-run here (Shopify-blind this session, only `.env.example` present) — carried as unverified secondary evidence, not a second probe pass. L9 (variant b default) matches `home-variant.server.ts` already on file from this tracker's own p1-7-frontdoor row. **Stock-indicator half of THIS milestone's own probe, independently re-verified this run**: `_layout.products.$slug.tsx` imports `StockIndicator` (L68), computes `stockIndicatorQty` (L846), and renders the component (L1023) — live, not commented out, and has not been since 2026-08-01 (PR #459). The tracker's own row text calling it commented-out was simply stale and is corrected here. Recording all of the above IS the "answers recorded in this tracker's status log" half of this milestone's DONE WHEN — but capping at AMBER rather than done because L5 is explicitly partial and two items (keyword_research/ ads_team valve rationale, and L5's scheduler-side confirmation) are still open, owner-only asks per blocker #119's own closing line. Not filed as a suggestion (owner-only, already on the blocker list). RED, unchanged for 8 straight audits -> now genuinely moving.
...
| p1-2-compare | `/compare/x-vs-y` route + Sanity comparison doc type + 3–5 seed comparisons | P1 | rr7-engineer + sanity-content-builder | 2026-08-10 | in-progress | AMBER | a compare route exists in `app/routes/` AND ≥3 comparison docs live with ItemList/FAQ schema + `.md` twins | 2026-09-22 | Sanity schema additive only. Audit 2026-08-17: infra half now shipped (suggestion #2389, applied) — `app/routes/_layout.compare.$slug.tsx`, `_layout.compare._index.tsx`, and both `.md` twins (`compare[.md].tsx`, `compare.$slug[.md].tsx`) exist, plus a real `comparison` Sanity doc type (`studio/schemas/comparison.js`, ItemList/FAQ-shaped). But `count(*[_type=="comparison"])` is 0 in Sanity, confirmed live — zero comparison docs have ever been published. Still RED: the code exists, the content doesn't. Suggestion #3892 filed (instructions, content) for a comparison-doc production quota; #2389 is terminal (applied) and doesn't cover this remaining content gap. Audit 2026-08-31: no Sanity API access this session (only `.env.example` present, confirmed via direct check), so `count(*[_type=="comparison"])` can't be re-run — unverifiable, stays capped, not flipped. Worth flagging regardless: this week's content run (590, 2026-08-30) published `/notebook/air-pulse-vs-clitoral-suction` under `blogPost.category:'comparisons'` as part of #3892's production quota — real comparisons content is now shipping, but as a Notebook `blogPost`, not the dedicated `comparison` Sanity document type this milestone's literal probe checks for (confirmed via `studio/schemas/comparison.js` vs. `blogPost.js`'s separate `category` field). Same definitional-drift shape as p1-7-frontdoor below: worth an owner/tech-architect call on whether Notebook comparisons content satisfies this milestone's intent, or whether the dedicated `/compare/` route + doc type remains the real target. RED, unchanged. Audit 2026-09-07: Shopify- and Sanity-blind again this session (only `.env.example` present), so `count(*[_type=="comparison"])` can't be re-run. Owner blocker #120 (filed 09-06, open) now asks directly whether shipping keep-both variants + Notebook comparisons satisfies this milestone and p1-7-frontdoor — the same definitional call flagged here for three straight audits is now on the owner's blocker list by name. RED, unchanged. Audit 2026-09-14: blocker #120 confirmed still open. Fresh live Sanity query this session (token available, not capped-unverified): `count(*[_type=="comparison"])` is still 0 — genuinely zero, not just unmeasured. Separately, Notebook `blogPost` content under the `comparisons` category continues shipping under #3892's quota: 10 live comparisons posts now (up from the single 08-30 post), and 27 guides+comparisons live overall (up from 15 at the 08-17 baseline) — real content velocity, still not the dedicated doc type this milestone's literal probe names. RED, unchanged. **Audit 2026-09-22 (program-manager, run 1021): RED -> AMBER, the definitional deadlock is broken.** Owner blocker #120 (researched 2026-09-20) settled the judgment call this row has carried since 08-24: **seed and KEEP `/compare`**, Notebook comparisons content does not retire it, both ship. Root cause of five weeks of zero output also diagnosed and fixed: ticket #3892's quota landed on the PLANNER (seo-curator) but never on the AUTHOR — neither `routine-content-daily.md` nor `.claude/agents/content-writer.md` had a procedure for the `_type:comparison` doc, so content-writer fell back to the Notebook post shape it knows every time. Ticket #10516 (instructions, target content) adds the authoring procedure and is confirmed `applied`. Companion ticket #10517 (sitemap coverage for `/compare/{slug}`) is `dismissed` — not independently re-diagnosed this run, worth a follow-up glance if a seeded comparison doc still doesn't show up in the sitemap once one exists. `count(*[_type=='comparison'])` not independently re-run this session (Sanity-blind, only `.env.example` present) so the milestone itself stays AMBER rather than done — the blocker is cleared and the fix is applied, but no comparison doc has confirmed-live evidence yet. Next audit: check the count directly once Sanity access is available.
...
| p2-1-pseo | Programmatic landing pages from mood/audience/matters × product_type_dial | P2 | rr7-engineer + seo-curator | 2026-09-14 | blocked | AMBER | a pSEO route template exists with ItemList/FAQ schema + `.md` twins AND pages index only past the quality bar | 2026-09-14 | blocked by p0-3-enrich. Audit 2026-09-14: target week reached (today). Zero code evidence — grepped `app/routes/` for `product_type_dial`/`mood`/`audience`/`matters`-combination route templates and found none; no dedicated pSEO route exists anywhere in the tree. GREEN(not-due)→AMBER rather than RED: the named blocker (p0-3-enrich's `moodTags` coverage gap, ~49% catalog-wide last measured, tracked under approved #1278) has both an owner and a path, and building pSEO templates ahead of tag saturation risks thin/fabricated combinations — the same reasoning this tracker already applies to p1-7-frontdoor's judgment-call AMBER. Not filed as a suggestion: the prerequisite itself is the blocker, not a missing engineering task. |
| p2-2-referral | Referral/loyalty execution: codes, rewards on `referrals` rows | P2 | loyalty-referral-manager + rr7-engineer | 2026-09-21 | not-started | RED | code generation + reward issuance paths exist against the `referrals` table | 2026-09-22 | Audit 2026-09-22 (program-manager, run 1021): target week reached (yesterday). Zero code evidence — grepped `app/lib/*.server.ts`, `app/routes/*.tsx`, and `scripts/*.ts` for any reference to the `referrals` table and found none outside `db/schema.ts`'s own definition. Direct query confirms 0 rows in `referrals` ever. No PR, no ticket, no named owner blocker covers this specifically. GREEN(not-due)->RED: past target with zero evidence and no path, this tracker's own established RED bar. Suggestion filed this run (`tracker:p2-2-referral`, code, strategy). |
| p2-3-social | IG/TikTok posting plumbing (approval-gated, voice-gated) | P2 | social-media-manager + rr7-engineer | 2026-09-21 | in-progress | AMBER | posting API clients exist for IG/TikTok, gated like X's double valve | 2026-09-22 | Audit 2026-09-22 (program-manager, run 1021): target week reached (yesterday), partial evidence. Instagram half is real and gated: `social-publish-job.server.ts`/`manual-publish-gate.server.ts` read `instagram_autopublish_enabled` fresh per post alongside the publish-gate stamp (X's own double-valve pattern). TikTok half does not exist: `social-publish-gate.server.ts:524-527` documents directly that TikTok is deliberately not yet a `GatePlatform` and the approve path 409s any tiktok row before it reaches the gate — a scoped, deliberate state, not a bug, and no TikTok posting client exists anywhere in `app/lib/`. GREEN(not-due)->AMBER rather than RED: real, gated IG plumbing is genuinely live, only the TikTok half is missing, and its absence reads as a prioritization call (does the store want to launch TikTok posting at all) rather than an unstaffed engineering gap — flagged as an owner/strategist judgment call in this week's brief rather than filed as a suggestion. |
...
| p2-10-tierc-trust-review | Tier-C vendor-trust review (carried-brand/allowlist condition?) | P1 | owner + product-manager | 2026-08-17 | done | GREEN | a dated decision is recorded in this status log re: whether to add a carried-brand/allowlist condition to Tier-C `autoImportPhase2` | 2026-09-07 | owner accepted "admit all vendors" 2026-07-13; revisit ~+30d (~2026-08-12); issue #255. Audit 2026-08-17: target week reached (revisit date ~08-12 already ~1wk overdue coming into this audit); no dated owner decision recorded in this log. GREEN(not-due)→RED. Owner-only policy call, not code/instructions-executable — flagged as an explicit owner ask in the weekly brief rather than filed as a suggestion row. Audit 2026-08-31: still no dated owner decision in this log. Revisit target was ~2026-08-12; today is 19 days past that, ~2.7 weeks overdue — correcting last audit's "~6 weeks overdue" note, which mis-measured against p0-7-valves' target rather than this milestone's own ~08-12 revisit date. RED, unchanged. Audit 2026-09-07: still no dated owner decision in this log. Owner blocker #109 (filed 09-05, open) now asks this exact question directly (carried-brand/allowlist condition, or reconfirm admit-all) — on the owner's blocker list by name rather than only in this tracker's carried note. RED, unchanged. Audit 2026-09-14: blocker #109 confirmed still open, no dated decision in this log. RED, unchanged. **DECISION 2026-09-20 (owner, interactive blocker review, blocker #109 cleared): ADMIT-ALL RECONFIRMED. No carried-brand or allowlist condition is added to Tier-C `autoImportPhase2`.** Measured basis: since the 2026-07-13 admit-all decision Tier C produced 13 candidates total (7 imported, 5 rejected, 1 pending) against 926 in Tier B and 138 in Tier D over the same window — under one admission per week. Everything admitted came from a recognizable carried brand (Jen Jenivive, Doxy, Tenga, Adam & Eve, three Tenga/iroha washes). All 5 rejections were Camtoyz torso SKUs, rejected by `product-manager-agent` on its own editorial judgment on 2026-07-31 — the exact filtering an allowlist was meant to supply, achieved without one, against a catalog spanning 140 distinct brands that would have to be listed and then maintained. The next revisit is deliberately a VOLUME trigger, not a calendar one: revisit if Tier-C admissions materially exceed the current ~0.7/week, or if any admission gets through that the owner would not have made. A 30-day calendar revisit on a lane this quiet is what produced six overdue audits. RED→GREEN, closed. |
...
### 2026-09-22 (program-manager, run 1021). Overall stays RED. Three RAG improvements (p0-7-valves, p1-2-compare both RED->AMBER; c-lane and design-elevation trackers separately closed real gaps), two new REDs (p2-2-referral, target week reached with zero evidence; p2-3-social partial-AMBER).

Re-probed all milestones with reached/approaching target weeks against files on `main`, direct Neon
queries, and this week's three owner-blocker research notes (#119 the L1-L9 checklist, #120 the
compare/frontdoor definitional call, #127 the review-invite backfill), corroborating what a direct
query could reach rather than taking any of the three at face value.

- **p0-7-valves: RED -> AMBER**, the cheapest unblock in the roadmap finally moves after 8 straight
  overdue audits. Stock-indicator half independently re-verified live (`_layout.products.$slug.tsx`
  L68/846/1023) — this tracker's own row text calling it commented-out was stale since 2026-08-01.
  L1-L9 checklist answers now recorded (see the milestone row): 8 of 9 answered, L5 (trigger
  existence in Claude's scheduler) stays partial, two valve-rationale confirmations stay open and
  owner-only. Not done yet, but no longer never-recorded.
- **p1-2-compare: RED -> AMBER.** Owner blocker #120 settled the five-week deadlock: seed and KEEP
  `/compare`, Notebook comparisons do not retire it. Root cause of zero output diagnosed and fixed —
  the authoring procedure landed on the planner, never the author; ticket #10516 (applied) fixes
  that. `count(*[_type=='comparison'])` not independently re-verified this session (Sanity-blind);
  next audit should confirm a live doc directly rather than carry this at AMBER indefinitely.
- **p1-7-frontdoor: stays AMBER**, narrowed. Blocker #120 now frames this as the whole remaining
  ask: write an ADR recording keep-both for the homepage front door (both `/` and `/discover`
  confirmed still live), or say the original single-winner mechanism is still required.
- **p0-4-reviews: stays RED**, bounded. Blocker #127 answered the webhook half (PR #1190/#9329,
  live since 09-15, future account-holder orders self-serve invites); the only remaining work is a
  one-time owner backfill of the 4 pre-fix historical order emails via an existing script.
  Suggestion #5233 is now stale against this evidence — flagged for the strategist to comment on
  and close, not re-filed.
- **p0-3-enrich: stays AMBER.** Live-draining-cohort healthy and current (enriched/published_7d
  both 138, both maxed today); `enrich_stuck` stable at 7. `moodTags` coverage stays UNMEASURED
  (Shopify-blind session) — owner blocker #119's secondary claim of near-saturation is noted but not
  acted on without an independent probe.
- **p2-2-referral: GREEN(not-due) -> RED.** Target week reached (2026-09-21) with zero code evidence
  anywhere in the tree and zero rows ever in `referrals`. No named blocker or owner path. Suggestion
  filed this run.
- **p2-3-social: GREEN(not-due) -> AMBER.** Target week reached with partial evidence: Instagram
  posting is real and gated (matches X's double-valve pattern); TikTok is deliberately not yet
  gate-eligible (documented in code, not a bug) — reads as a scope/priority call for the owner, not
  an engineering gap, so not filed as a suggestion.

**Suggestions filed this run**: `tracker:p2-2-referral` (code, strategy). **Suggestions flagged
stale for the strategist to close, not re-filed**: #5233 (review-invite pipeline, superseded by
blocker #127's diagnosis).

**Routine-coverage check** (independently re-derived from `routine-schedule.md`'s 26-row manifest,
2 exempt: R-WATCH #22, R-SHEP #23; shared with self-healing-automation.md per this tracker's own
convention). **One correction to this run's own backstop finding**: routine 10 (Weekly SEO
Curation) did NOT miss. Direct query on `homepage_team_runs` shows genuine seo-curator work on both
2026-09-13 (run 847: gray-zone triage, cluster hygiene, trend review, 3 new `seoContentBrief` docs
planned) and 2026-09-20 (run 970: triage, clusters, 7 pending trend briefs reviewed, weekly report
posted) — both landing at the Sunday 10:00 UTC cadence the manifest specifies — but both runs are
logged with `run_type='manual'` instead of `run_type='seo-curation'`, which is exactly what breaks
any check (this run's own backstop included, and suggestion #10872's diagnosis) that groups runs by
`run_type`. Suggestion #10872 (`routine10-seo-curation-not-firing`, approved, non-terminal) names
the wrong root cause — a NULL git-source outage, the documented failure signature at
`routine-schedule.md` row 25 — when the evidence shows the routine firing and producing real,
on-cadence output; the actual defect is a `runType` mislabeling somewhere between the live trigger
and `POST /api/team/run` (the playbook itself correctly specifies `runType:'seo-curation'` at
`routine-seo-curation.md:36`). **Not re-filed** (non-terminal, same dedupe rule as everywhere else)
— flagging for the strategist to comment on #10872 with this correction so R-DEV does not spend a
claim confirming a source that is not actually null. Routine 13 (Daily Pricing Sweep) stays
unmeasured by design (no team gate, structurally no run row possible) — already covered by #10873,
not refiled. All other 24 non-exempt lanes confirmed alive this week via direct run-row or cron-run
evidence; zero half-enabled instances (R-BLOCK's manifest-row gap from prior audits is now closed,
row 26 present).

**Asks for the owner**: (1) the two items p0-7-valves' checklist still leaves open — confirm L5
(trigger existence) in the routines UI, and confirm whether `keyword_research_enabled=false` and
`ads_team_enabled=false` are still intentional. (2) run the review-invite backfill script for the 4
historical orders (steps are in the p0-4-reviews row and blocker #127). (3) write or direct an ADR
for the keep-both homepage front-door decision (p1-7-frontdoor / blocker #120). (4) p2-3-social:
confirm whether TikTok posting is in scope at all before it is worth engineering time.


### 2026-09-20 (interactive owner blocker review). One RAG change: p2-10-tierc-trust-review closes GREEN on a dated owner decision.

Not a program-manager audit. This entry records owner decisions taken in an interactive blocker
review so they stop being invisible to this tracker, which is the thing p2-10 has been waiting on
for six straight audits.

- **p2-10-tierc-trust-review (RED→GREEN, closed).** Owner reconfirmed **admit-all** for Tier-C
  `autoImportPhase2`; no carried-brand or allowlist condition is added. Full measured basis is in the
  milestone row. The short version: 13 Tier-C candidates in ten weeks, every admission from a
  recognizable carried brand, and all 5 rejections (Camtoyz torso SKUs) already made correctly by
  `product-manager-agent` without an allowlist. Next revisit is a volume trigger, not a date.
- **Owner blocker #109 cleared** on that decision. Blockers #85, #110, #147 were also cleared in the
  same review, and #118 was cleared as a false alarm (`/cron/gsc-snapshot` has in fact fired every
  Monday for eight straight weeks; the investigation had checked `cron_runs`/KV, which that route is
  declared `recorded: false` and never writes).

**Correction to this tracker, carried from the same review:** the `p0-7-valves` row still states the
PDP stock indicator is commented out at `_layout.products.$slug.tsx` L61 and L867-868. It is not, and
has not been since 2026-08-01 (commit `5698509c`, ticket #415, PR #459). Live wiring is the import at
L68, `stockIndicatorQty` at L846, and the component rendered at L1023. That half of the milestone's
DONE WHEN already passes. The 2026-08-24 audit note had this right; the header row was never updated.
Not corrected in this PR to keep it to one decision — flagged for the next program-manager pass.

### 2026-09-14 (program-manager, run 865). Overall stays RED. One RAG change (p2-1-pseo, target week reached); p0-3-enrich's active break resolves; owner blockers spot-checked, all still open.

Re-probed all 25 milestones against files on `main`, live Neon queries (`pipeline_settings`,
`import_candidates`, `review_invites`, `homepage_team_suggestions`, `owner_blockers`), a live Sanity
query (token available this session, unlike several recent audits), and a PII-free Shopify Admin
order-list query. One row changes:

- **p2-1-pseo: not-started/GREEN(not-due) → blocked/AMBER.** Target week is today (2026-09-14).
  Grepped `app/routes/` for any mood/audience/matters × product_type_dial route template — none
  exists. AMBER rather than RED because the named blocker (p0-3-enrich's `moodTags` coverage gap,
  tracked under approved #1278) genuinely has an owner and a path, matching this tracker's own
  precedent for judgment-call AMBERs (p1-7-frontdoor). Not filed as a suggestion: the prerequisite
  is the blocker, not a missing engineering task.

**p0-3-enrich's active break from last week is resolved.** #7993 (Nalpac feed Brand-column outage)
cleared 2026-09-11 per this run's product-manager sub-step. The live-draining cohort is fresh again:
`max(enriched_at)` = today, `max(published_at)` = yesterday (normal lag), enriched_7d 93,
published_7d 69, `enrich_stuck` down to 7 from 29. `moodTags` catalog coverage was not re-measured
this session (Shopify token was available but a full-catalog metafield sweep was out of this audit's
time budget) — carried UNMEASURED at ~49.3%, still capped AMBER, not guessed.

**Four carried REDs re-confirmed with fresh evidence, no flips:** p0-4-reviews (`review_invites`
still 0 rows ever; Shopify order list — no PII fields queried — confirms still exactly 4 lifetime
orders, most recent still #1004 from 09-07, no new order this week; blocker #127 open); p0-7-valves
(blocker #119 open, L1-L9 still never recorded, now 8 weeks past target); p1-2-compare (fresh live
Sanity query this session, not capped-unverified: `count(*[_type=="comparison"])` is genuinely 0;
separately, Notebook `blogPost` comparisons content keeps shipping under #3892's quota — 10 live
comparisons posts now, 27 guides+comparisons overall, up from 15 at the 08-17 baseline; blocker #120
open); p2-10-tierc-trust-review (blocker #109 open, no dated decision). p1-7-frontdoor and p0-1-email
also re-confirmed unchanged AMBER (blockers #120/none respectively; email campaign rows still 4
approved, `email_campaign_push_enabled` still absent).

**Regression spot-checks on `done` rows:** p1-1-guides (live Sanity count now 27 guides+comparisons,
up from 15 — improving, not regressing), p1-3-plp-copy (still 172/172 `collectionPage` docs with
`introCopy`), p0-8-links (still dead code inside the unreachable legacy branch, stays dropped). No
regressions found on any spot-checked row.

**Routine-coverage check** (scope derived from `routine-schedule.md`'s 25 numbered routine rows,
last 7 days, cross-checked directly against `homepage_team_runs`): **0 no-run misses** — every one
of the 25 rows produced at least one run row this week, including confirming both `content/manual`
routines (SEO curation run 847 on Sun 09-13, Podcast Review run 774 on Wed 09-09) fired distinctly
under the shared `run_type='manual'` tag. **1 half-enabled instance, and it is NOT the same shape
as last week's**: `strategy/blocker-scout` (R-BLOCK) fired 5 times this week (succeeded 09-08
through 09-11, one skip 09-12) under `blocker_scout_enabled` (ON) — genuinely alive, not dead. But
`docs/store-team/routine-schedule.md`'s own numbered table still carries no row for it (checked by
direct grep, zero matches for "R-BLOCK" or "blocker-scout" anywhere in the file). Last week's
suggestion #7667 (`unwatched-lane-strategy-blocker-scout`) is now `applied` (terminal) — but it only
added `ROUTINE_CADENCES` entry in `ticket-janitor.server.ts`, a monitoring safety net, not a
manifest row in `routine-schedule.md` itself, so the specific gap this run's audit scope is checking
(the manifest the program-manager derives scope from) is still unrepresented. Since #7667 is
terminal, this is a fresh finding, not a duplicate — filed this run (see suggestion payloads,
targeting `docs/store-team/routine-schedule.md`, kind `instructions` not `process`: the fix is a
docs edit with a real executor, agent-editor).

No new suggestions filed for this tracker's own milestones this run — every remaining RED/AMBER gap
is already covered by a non-terminal prior row or is an owner-only decision already on the blocker
list by name (#109, #119, #120, #127). One suggestion filed this run for the routine-coverage
finding above (not a tracker milestone).

**Asks for the owner (carried, all confirmed still open on the blocker list):** (1) #119,
p0-7-valves L1-L9, now 8 weeks overdue, cheapest unblock in the roadmap. (2) #120,
p1-2-compare/p1-7-frontdoor definitional call. (3) #109, p2-10-tierc-trust-review carried-brand
condition. (4) #127, confirm order #1003/#1004 customer email for review invites. (5) The tracker-PR
backlog on the remote is now 8 unmerged weekly `pm/tracker-*` branches (07-20 through 09-07) — worth
a batch look.


### 2026-09-07 (program-manager, run 736). Overall stays RED. No RAG changes; one fresh, active break found on p0-3-enrich, and three carried gaps now sit on the owner's blocker list by name.

Re-probed all 25 milestones. No Sanity or Shopify credentials available this session (confirmed via
direct `.env*` check — only `.env.example`), so the Sanity/Shopify-gated probe halves stay unverified
and capped, unchanged from the last several audits.

**p0-3-enrich (AMBER, unchanged) has a new, active, already-tracked break worth flagging plainly.**
The live-draining-cohort half of the probe has genuinely stopped draining for two straight days
(max enriched/published both stuck at 2026-09-05) despite the enrich cron reporting `succeeded` on
09-06 and 09-07 — a routine that reports healthy while producing nothing. Root cause is identified and
already on the bus: ticket #7993 (approved, no PR), the Nalpac feed's Brand column has been empty
catalog-wide since ~09-05, so master-matching fails for every import candidate. Not refiled.

**Three carried REDs/AMBER now have direct owner blockers naming them**, rather than only living in
this tracker's own carried notes: p0-7-valves (#119), p1-2-compare + p1-7-frontdoor (#120, one blocker
covering both), p2-10-tierc-trust-review (#109). No change in RAG for any of them — they were already
the right color — but each now has a more visible surface than a weekly tracker row, worth noting so
next week's audit checks the blocker list first.

**Also worth recording:** the store's second real non-owner order landed this week (#1004, 09-07,
Shawn Volk, $19.99) — 4 lifetime orders, 2 real customers, still zero review invites sent against
either (p0-4-reviews).

**Routine-coverage check**: see this week's self-healing-automation.md entry for the full scope
derivation (25 rows, 0 no-run misses, 1 half-enabled instance already covered by a live suggestion) —
not duplicated here to avoid two conflicting counts in two files.

No new suggestions filed for this tracker this run — every remaining gap is either already covered by
a non-terminal prior row (#7993 for p0-3-enrich, #5233 for p0-4-reviews) or is an owner-only decision
now sitting on the blocker list by name (p0-7-valves, p1-2-compare, p1-7-frontdoor,
p2-10-tierc-trust-review).

**Asks for the owner (carried, now cross-referenced to open blockers):** (1) #119, p0-7-valves L1-L9,
7 weeks overdue, cheapest unblock in the roadmap. (2) #120, p1-2-compare/p1-7-frontdoor: accept
keep-both variants + Notebook comparisons as satisfying these milestones, or hold out for the
originally-specified mechanism. (3) #109, p2-10-tierc-trust-review: carried-brand/allowlist condition
on Tier-C auto-import, or reconfirm admit-all. (4) The tracker-PR backlog on the remote keeps growing
(now 7 weekly `pm/tracker-*` branches (07-20 through 08-31) unmerged, per this week's PR — see the release-engine
eligibility note there) — worth a batch look.
