...
| 15 | xdipx — Weekly Video Producer | `trig_01QBLBTi9sS7X7FjFXAvPfkw` (created 2026-08-16 at enablement, `0 17 * * 2`, enabled; `video_team_enabled` had been flipped on with no trigger, so the routine had zero runs ever) |
...
| 6 | xdipx — Social Drafts | `0 14 * * *` and `0 22 * * *` (twice daily) | social / `social-drafts` | `docs/store-team/routine-social-daily.md` | DRAFT-ONLY: every draft passes the emma-empathy-reviewer voice gate, then lands as a `social_posts` row with status `draft`. The routine never posts; on Instagram the independent `social-publish-gate` decides at Step 6.5 and the hourly `/cron/social-publish` job ships what it approved while `instagram_autopublish_enabled` is on. Never touch a valve. **The second firing was added 2026-08-16** when the owner raised `social_freq_instagram` to 3: one trigger is one drafting pass, so a single run produces the day's posts in one cluster, and the evening run also fires after the content routine publishes, which makes it the run that can see the day's Notebook promo row. **The quota is per day, not per run**: the evening prompt leads with that, because two runs each drafting a full quota is double the intended volume. Both firings count against `social_team_max_runs` (4), which leaves one spare run on Monday (trend scout 17:00) and Thursday (business research 16:00). |
