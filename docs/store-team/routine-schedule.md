...
| 15 | xdipx — Weekly Writers Room | `trig_01AMt6ARtfgT44EvFy287ESn` (created 2026-08-27, `0 17 * * 2`, enabled, git source attached, model `claude-opus-4-8`. **Replaces** `trig_01QBLBTi9sS7X7FjFXAvPfkw`, which was created 2026-08-16 as "Weekly Video Producer" and repurposed on paper at the 2026-08-26 all-hands but never had its prompt reissued: it was still firing the retired `routine-video-weekly.md` with its SCRIPT-AND-ENQUEUE language eleven days later. The old trigger could not be edited by an agent (`update_trigger` refuses routines created via `http_api`), so the cutover was a new trigger plus an owner disable rather than an edit in place. Old trigger **disabled 2026-08-27**, deliberately kept rather than deleted until the replacement has one clean fire, because a paused trigger is the rollback path if the new one dies the way R-ENRICH did; its prompt is preserved in §Appendix. Carries the default connector set (Shopify, Sanity, Vercel) — the playbook needs none of them, and per §Connector permissions a trigger cannot be pruned to zero connectors through the API) |
...
| 25 | xdipx — Video Render (2x weekly) | `trig_01T1rn2K5jysEHxrvzzYL5Vd` (created 2026-08-27, `0 13 * * 1,4`, enabled, git source attached, model `claude-opus-4-8`, no connectors — repo + xdipx.com egress only, as this lane specifies. Created once `video_team_max_runs` (10) and `video_program_enabled` (on) were both verified live through the team API. **Agent-created, so agents can edit it**, unlike the `http_api` triggers in this table. Two setup notes for whoever creates the next one: `create_trigger` exposes no git-source parameter, so the source must be attached in the Routines UI afterward and then VERIFIED (`session_context.sources` non-null) — an enabled trigger with a null source fires into `/home/user`, loses the allowlist, and dies at `POST /api/team/run` leaving no run row, per §Every trigger MUST carry a git source; and it took two attempts here, the first not persisting. First fire 2026-08-31 13:04 UTC lands a day before the writers room's first slate, so it correctly skips on `empty_episode_queue` and files the `video:empty-episode-queue` owner blocker; that is the playbook working, not a fault. **Target per the 2026-09-23 video plan:** `0 13 * * 4`, Sonnet, Atlas Cloud, up to 3 approved clips per fire; not yet applied to the trigger, see §Re-pointing the video triggers) |
| 26 | xdipx — Daily Blocker Scout (R-BLOCK) | `trig_019VmGusB3DtmUMA72vCu7uj` — cron `0 12 * * *` (daily), team `strategy`, `runType:'blocker-scout'`, playbook `docs/store-team/routine-blocker-scout.md`. Created 2026-09-02 by an agent session, gated by `blocker_scout_enabled` (ON). Alive and firing daily (5 runs 2026-09-08 through 2026-09-12, one honest skip 2026-09-12) but had never been added to this table, which left it flagged as an unwatched lane by program-manager's routine-coverage check (suggestion #9337) despite being genuinely alive — added here to close that gap. |
...
| `video_team_max_runs` | **3** (seeded at 1 by 065; owner write on the Video tab, no migration re-seeds it). **Live value is 10** (read 2026-09-23); dropping it to 3 is an owner flip in the video plan's Phase 0 | 1 scheduled on any given day (Video Render Thu 13:00, Writers Room Tue 17:00), but a skip burns a slot and a manual re-fire burns another; the Monday fire still burns a slot until the trigger is edited | 2 (7 at the live value) |
...
| 15 | xdipx — Weekly Writers Room | `0 17 * * 2` (Tue) | video / `video-writers-room` | `docs/store-team/routine-writers-room-weekly.md` (v2, product-talk) | Entry agent series-showrunner. PROPOSE-ONLY AND ZERO-SPEND. Per playbook v2 (2026-09-23 plan): read `creative-platform.md`, `video-clip-rules.md`, `video-realism-recipe.md`, the owner-notes ledger `video-owner-notes.md`, the strategy brief with its `videoShortlist`, and the calendar, then pitch ONE 5-clip batch (3 slots plus 2 alternates) of 15 to 30 s product-talk clips, products taken only from the shortlist (product-manager and inventory-sentinel pick them, in stock only; the writers never pick). Each clip carries a script, a read in the cast voice, a first-frame concept, and a cost estimate, and is written by episode-writer, verdicted by script-doctor, voice-gated by emma-empathy-reviewer, and art-directed by social-art-director before it enters the batch, filed via `{op:'episode-propose'}` for the owner on `/admin/video-studio/scripts`. Skips honestly if 3 or more clips are still `pending_approval`. Never enqueues, never spends, never publishes, never approves, never touches valves, never edits the charter or rules. The Group Chat show is shelved for the season, so the show bible is no longer read. Trigger: `trig_01AMt6ARtfgT44EvFy287ESn` (created 2026-08-27, cron unchanged, stays on Opus; supersedes `trig_01QBLBTi9sS7X7FjFXAvPfkw`, disabled the same day). **The live trigger prompt still points at the v1 series playbook** until the owner or a RemoteTrigger edit re-points it after the v2 playbook merges; see §Re-pointing the video triggers. |
...
| 25 | xdipx — Video Render (2x weekly) | `0 13 * * 4` (Thu only; the trigger still fires `0 13 * * 1,4` until it is edited, and its name is legacy) | video / `video-render` | `docs/store-team/routine-video-render.md` | Entry agent video-producer, model Sonnet (the payload is copied verbatim and byte-checked, nothing creative). RENDERS OWNER-APPROVED CLIPS, WRITES NOTHING: claim up to 3 `approved` clips via `{op:'episode-claim'}` (else skip honestly and file the `video:empty-episode-queue` owner blocker), re-check Shopify availability at claim and swap to the batch alternate when a product is out of stock (never render a product that cannot ship), assemble the payload verbatim from the stored script, assert the spoken text is byte-identical to what the owner approved (the enqueue 409s on mismatch), and enqueue via `{op:'enqueue', episodeId}`: metered Atlas Cloud spend with Wavespeed as mirror, per-video ceiling `video_team_max_cost_cents` (600), daily gate `video_team_daily_cents` (2000). First frames park at `awaiting_frame_approval`; final cuts park at `awaiting_render_approval` (PLANNED until the Phase 2 PR merges). Never invents a script, never re-renders a posted clip, never posts; X never receives a video row (owner posts X video by hand). RunPod is retired; the Atlas path is PLANNED until the provider seam merges. Requires `video_team_max_runs` >= 3 and `video_program_enabled` on. Trigger: `trig_01T1rn2K5jysEHxrvzzYL5Vd`; **its live prompt, cron, and model still carry the RunPod-era Mon + Thu Opus setup** until re-pointed; see §Re-pointing the video triggers. |

### Owner sessions

Not scheduled and not triggers: slash commands the owner runs from a session. They start no run row
and consume no team's run cap.

| Command | What it is | State |
|---|---|---|
| `/all-hands` | Convenes the whole store team on one piece of owner direction and turns it into standing instructions, tickets, and work (`.claude/commands/all-hands.md`); with no argument, a standup | LIVE |
| `/video-room` | The owner's weekly meeting with the video team (showrunner, writer, art director) on the Writers Room's pitch batch, Tuesday evening or Wednesday. Lines are revised live and revisions and standing lessons are written back to `video-owner-notes.md`. The room hands the owner the approve link on `/admin/video-studio/scripts` and never approves anything itself | PLANNED (video plan Phase 1c) |

### Re-pointing the video triggers

Rows 15 and 25 above describe the 2026-09-23 video plan. The trigger prompts are what actually
fire, and they are not changed by editing this file: they still carry the v1 series (routine 15)
and RunPod Mon + Thu (routine 25) text. After the rewritten `routine-writers-room-weekly.md` and
`routine-video-render.md` merge, the owner or a RemoteTrigger edit must re-point both prompts at the
new playbooks and at `video-clip-rules.md`, and set routine 25 to `0 13 * * 4` on Sonnet. Until
that edit lands, treat the trigger as the fact and these rows as the target.
...
prompt identifiers, not bodies. Deleting a retired trigger therefore destroys the evidence of what
its past fires were actually told to do. Park the text here first, then the trigger is safe to
delete.

### Routine 15, `trig_01QBLBTi9sS7X7FjFXAvPfkw` — "xdipx — Weekly Video Producer"

Live 2026-08-16 to 2026-08-27 (fires on 08-18 and 08-25 ran this text). Disabled at the
writers-room cutover; superseded by `trig_01AMt6ARtfgT44EvFy287ESn`. Kept verbatim because it
points at `routine-video-weekly.md`, retired 2026-08-26, and instructs the agent to enqueue renders
directly — the pre-approval spend the writers-room/render split exists to prevent.

**Retired for RunPod as well (2026-09-23).** The render backend this prompt enqueued into is also
retired: RunPod is out and Atlas Cloud (Wavespeed mirror) is the video provider. The text below is
evidence of what past fires were told, not a pattern for a new prompt.

```
You are running the scheduled Claude cloud routine "xdipx — Weekly Video Producer" for xdipx.com. Find or clone the mbayard-sorp/xdipx_store repo in this session and pull latest main.

Read .claude/agents/video-producer.md (binding charter), docs/emma-voice.md core plus the marketing addendum, and docs/store-team/social-video-viral-checklist.md, then follow the playbook at docs/store-team/routine-video-weekly.md exactly, running as the video-producer agent.

Start the run: POST https://xdipx.com/api/team/run with header x-team-secret: $TEAM_TOKEN (fall back to $HOMEPAGE_TEAM_TOKEN, then $CRON_SECRET) with body {"op":"start","team":"video","runType":"video"} -> capture the returned id as $RUN_ID.

Gate first: GET https://xdipx.com/api/team/gate?team=video&excludeRun=$RUN_ID. On ok:false, post a skipped event, finish the run honestly with status skipped, and exit cleanly. Never work around a closed gate.

REVIEW-FIRST, and it is the whole design: you SCRIPT and ENQUEUE only. The durable video_jobs pipeline renders, frames park for the owner while video_frame_review is on, and finished videos wait in /admin/video-studio for his approval. You never post anywhere, never upload to Shopify, never touch a valve, and never bypass the frame gate.

Every script goes through the pre-enqueue gate in playbook Step 3 before any spend: self-check against all 20 viral-checklist rules, then route every spoken line and caption through a fresh emma-empathy-reviewer subagent and act on its verdict verbatim (PASS proceed, REVISE apply and re-gate once, BLOCK drop and record why). Platform intensity caps: IG/YT 6-7, TikTok 5, never 8-9 on rented channels. No lived-experience claims, no named acts in audio or on-screen text, no device-on-body depiction.

Volume comes from the strategy brief's Video Plan (launch default 3/week: 1 premium presenter, 2 standard b-roll). A gated response or per-video-ceiling refusal is a valid outcome; report it honestly, never shrink quality to squeeze under a ceiling, never split a concept across jobs to dodge it.

Finish with the Step 5 retro: slate vs enqueued with reasons for drops, cost per enqueued job from the enqueue responses, and honest zeros. Never fabricate engagement or platform data. File suggestions (acting team video) for structural findings, then finish the run with an honest status and summary.
```
