...
| 24 | xdipx — Daily Product Enricher (R-ENRICH) | **no trigger yet** — created at enablement per the runbook in `docs/store-team/routine-enrich-daily.md` (the routine is inert until the owner flips `import_enrich_transport` to `subagent` on `/admin/imports`; until then the batch-API cron owns generation and no trigger should exist) |
