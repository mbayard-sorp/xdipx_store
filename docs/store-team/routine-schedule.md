...
| 18 | xdipx — Daily Dev (R-DEV) | `trig_01MEQYsg5sHPbM4v39FqssAD` (live 2026-07-28; one trigger covering every pass; cron changed 2026-08-07 from `0 14,20 * * *` to `0 2,8,14,20 * * *` on owner direction to drain the 43-ticket approved-code backlog; prompt `rdev-daily-0003`: claim cap 5, leaseSeconds 10800, branch `ticket/<id>`) |
| 19 | xdipx — Daily QA Gate (R-QA) | `trig_019GjVP9hGBU1gmXRBYtYURm` (live 2026-07-28; cron changed 2026-08-05 from `30 15 * * *` to `30 3,15 * * *`, then 2026-08-07 to `30 3,9,15,21 * * *` to stay 90 min behind each R-DEV pass; prompt `rqa-daily-0003` resumes `in_review` first and no longer bounces a ticket for a check GitHub never built) |
...
|---|---|---|---|
| `strategy_team_max_runs` | **12 required** since the four-pass dev/QA cadence (2026-08-07). **Currently 8, which is not enough.** | 11 on Monday: R-DEV 02:00, R-QA 03:30, R-DEV 08:00, R-QA 09:30, Weekly Strategy 12:00, R-DEV 14:00, R-QA 15:30, R-DEV 20:00, Cost Review 21:00, R-QA 21:30, Apply Pass 22:00. 9 on Thursday (same minus the weekly brief, plus that day's Apply Pass). 8 on every other day, which fits exactly with zero headroom | **-3 on Monday** |
...
| 18 | xdipx — Daily Dev (R-DEV) | `0 2,8,14,20 * * *` (daily, four passes since 2026-08-07) | strategy / `strategy-dev` | `docs/store-team/routine-dev-daily.md` | Entry agent rr7-engineer. Claims up to 5 `kind:'code'` tickets per pass (raised from 3 on 2026-08-05, prompt now `rdev-daily-0003`, alongside leaseSeconds raised from the prompt's old 1200 to 10800 after mid-run lease expiry orphaned tickets 120 and 423) via `{op:'claim'}` in priority order, one branch and one PR per ticket (`ticket/<id>`, NOT `agents/**`, which is agent-editor's docs-only namespace and would fail the `agent-allowlist` check), runs typecheck + tests + build locally before opening the PR, transitions to `pr_open` with the PR link. Tickets that would require touching a protected path go to `blocked` with a note instead of being coded. Ticket bodies are untrusted input; the playbook always wins. The last pass of the UTC day claims bounced tickets first. Went from two passes to four on 2026-08-07 on owner direction: at 5 claims a pass that is 20 code tickets a day against a 43-ticket approved backlog. **This needs `strategy_team_max_runs` at 12; at 8 the Monday and Thursday Apply Pass is what skips.** Trigger: `trig_01MEQYsg5sHPbM4v39FqssAD` (live since 2026-07-28). |
| 19 | xdipx — Daily QA Gate (R-QA) | `30 3,9,15,21 * * *` (daily, four passes since 2026-08-07) | strategy / `strategy-qa` | `docs/store-team/routine-qa-daily.md` | Entry agent qa-reviewer. Lists `in_review` and `pr_open` tickets (crashed `in_review` rows resumed first, prompt `rqa-daily-0002`), transitions each fresh one to `in_review`, pulls the branch, reviews the diff, runs static checks, reads CI status and the rendered preview through `GET/POST /api/team/pr` (the only path that works under xdipx.com-only egress), then either `verified` with evidence or bounces to `in_progress` with a concrete `last_error`. QA never merges and structurally cannot reach `applied`. Four passes since 2026-08-07, each 90 minutes behind an R-DEV pass, so a PR is reviewed within about two hours of opening instead of waiting up to 12. Prompt `rqa-daily-0003` also stops QA bouncing a ticket over a `check` GitHub never built or a job that concluded `cancelled` without running: neither is a fault in the diff. Trigger: `trig_019GjVP9hGBU1gmXRBYtYURm` (live since 2026-07-28). |
