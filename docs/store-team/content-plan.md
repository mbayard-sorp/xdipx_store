...
**Structured comparison-doc quota: seed 1 per week until 3-5 exist (added 2026-08-18, #3892).** This is the `_type:comparison` Sanity doc that powers the `/compare/{slug}` route (with its ItemList/FAQ schema and `.md` twin), which is a **different artifact** from the Sunday `blogCategory:comparisons` Notebook post above. The compare infra shipped (route, `.md` twins, and the `studio/schemas/comparison.js` doc type all live on `main`, suggestion #2389 applied) but `count(*[_type=="comparison"])` is still 0, so tracker milestone `p1-2-compare` sits RED for lack of content, not code. Until at least 3 (target 3-5) `_type:comparison` docs are published, seed **one per week**: the seo-curator's Sunday planning (`routine-seo-curation.md` Step 5) picks the head-to-head from the same comparison queue it already reads and hands it to the content lane to author as a `comparison` doc (honest in-stock embeds, both sides real), rather than only ever producing the Notebook `comparisons` post. Once 3-5 are live the quota lapses and comparison work returns to the normal Sunday cadence. Never fabricate a comparison to light up the route; if no honest head-to-head is available a week, say so and skip.
