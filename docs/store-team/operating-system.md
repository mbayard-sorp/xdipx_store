...
|---|---|---|---|---|
| hourly, :25 | `/cron/log-monitor` | Vercel cron | Files a ticket at the owning lane for each recurring class of auto-expired team run. The Haiku log classifier this route is named for was deleted 2026-09-02 (433 calls, 16.9M input tokens, 30 days, zero log-derived tickets in its lifetime); Sentry already covers runtime errors natively | LIVE |
| every 15 min | `/cron/conversion-watch` | Vercel cron | Conversion-delivery watcher plus CAPI reconciliation. Split off `/cron/log-monitor` 2026-09-02 so that dropping the classifier route to hourly could not silently take a P0 money-path check with it | LIVE |
...
| every 6 h (:20) | `/cron/social-metrics-sweep` | Vercel cron | Refreshes `metrics_json` on posted Instagram and X rows from the last 30 days and appends an IG follower reading, so the Social Studio analytics view reads fresh numbers. Read-only against both platforms. X bills per metrics read, so it is gated by `social_metrics_sweep_enabled` (default OFF) and capped by `x_metrics_max_reads_month` (default 1500); both are owner-only spend controls on the Social tab of `/admin/homepage-team` | SHIPPED INERT (Social Studio v2 Phase 6b, ticket #4916); valve flip is the owner's |
