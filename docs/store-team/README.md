...
| `instagram_autopublish_enabled` | `false` | **Publish valve (Instagram).** On = the hourly `/cron/social-publish` tick posts approved, due Instagram drafts. Capped by `instagram_publish_max_per_day`. Scoped to the unattended tick: the owner's Post-now click publishes a still with this off (owner direction 2026-08-23), the same as X. |
