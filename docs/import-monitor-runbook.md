...
8. **Rank + cap.** Rank all eligible masters by `gap_score` DESC, keep the top `import_monitor_max_candidates` (default 300). A live run collapsed ~15,500 masters → ~3,400 eligible → 300. The full strategic view stays in the Catalog Opportunities report. (Note: a single-variant Tier A "proven seller" can score below deep multi-variant Tier B masters and fall outside the cap — see §6 if you want Tier A guaranteed a slot.)
