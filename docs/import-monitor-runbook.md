...
- **Actual cadence is Mon/Wed/Fri, not daily.** The cron fires every day, but the handler exits early on days outside `import_monitor_run_days`, which is `1,3,5` in production (`server/cron.ts:533` reads the valve and returns `{ok: true, skipped: true, reason: 'not_scheduled_today (day=N)'}` — note `ok: true`, so a skipped day is not distinguishable from a run by status code alone). The vercel.json schedule alone therefore overstates how often discovery runs, and anything downstream that assumes a fresh queue every morning — notably the 09:00 UTC daily product routine — works from the last discovery pass on Tue/Thu/Sat/Sun. The value is an unversioned production hand-edit; no migration seeds it.
