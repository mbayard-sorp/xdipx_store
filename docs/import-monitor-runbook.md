...
7. **Tier + score + price.** Per master: tier label (A top-100 / B carried-brand ≥45% / C new-feed ≥50% / D uncarried opportunity) for the "why"; `gap_score` (rewards margin × variant count × stock depth, stored in `deal_score`); MAP-safe price preview via `computeTargetPrice()` on median pricing.
8. **Rank + cap.** **Tier A (Nalpac top-100) always surfaces** — proven sellers we don't carry belong on the site, so they bypass the cap. Remaining slots go to the highest `gap_score` non-A masters, up to `import_monitor_max_candidates` (default 300). A live run collapsed ~15,500 masters → ~3,400 eligible. The full strategic view stays in the Catalog Opportunities report. (Tier C "new releases" still compete in the gap_score pool.)
