...
use — pass `'4:5'` for a feed/carousel still). `image_urls` order does not matter to the
model.
...
Instagram carousel image rejected as taller-than-4:5. Center-crop-then-resize every candidate to
exactly 1080x1350 before the vision gate and before upload; don't trust the returned dimensions.
