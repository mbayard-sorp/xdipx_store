...
**Fields**
- `eyebrow` (string) — mono kicker, e.g. "Curated by Emma, your AI wellness guide".
- `headline` (text, ≤80 chars) — Newsreader; one word wrapped in `<em>` for the plum-italic treatment.
- `emmaAside` (text, ≤120 chars) — Emma first-person advisory; AI-guide voice, never lived experience.
...
`app/components/seo/FAQStructuredData.tsx`). Answer-shaped, citable copy for AEO. Emma voice; no
em-dashes, no countdowns.
