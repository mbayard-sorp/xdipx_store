...
### 2026-08-14 — Routine A — Hero rotated to b-Vibe Snug Ring (owner-directed, run 314) + design-critic REVISE fixes
- **What:** Rotated the hero from the We-Vibe "Moving as One" kit to the **b-Vibe Snug Ring 45mm black**. Promoted the clean device sibling `98863B` to primary via `setMediaAsPrimary` (the raw primary `98863A` was a blister-pack composite), pinned `featuredProductHandle` + `primaryCtaLink`, and wrote dial-9 hero copy act-anchored to the worn c-ring. Fresh-art floor met: 3 generated images (promo `485a2530` coral-soft, couples band `404932a4` plum-soft, couples wayfinder tile `d7247419` paper), grounds rotated across the lock, all §2 self-reviewed. Ran the **design-critic** spot-check (Step 7.5): REVISE, avg 3.75 (imagery 4.5, motion 4.5 strong; hierarchy/type flagged). Applied its content fixes — shortened the hero headline to a single line and the eyebrow to a true mono kicker, moved the long copy to the body. Filed the two component-level defects (Sanity hero headline renders plain with no plum `.em` split; Ask-Emma H2 emphasis word is coral-2 not plum) as code ticket #3197 for Routine B.
- **Why:** Owner direction (ticket #3184, then a live "kick off the rotation now"). Active theme "Weekend In, Not Out" (couples + wearables), `marketing_calendar` #6. Step 7.5 design gate.
- **Evidence:** run 314; hero handle + clean LCP `98863B` (eager) + new headline verified at origin, packaging `98863A` gone, `rails:4` team content (not fallback); Sanity hero copy rev `J5AMcOqaNUY9YyUo4PFsWA`; design-critic verdict event (avg 3.75, REVISE); Shopify Storefront reorder + singleton copy each needed a second warm past eventual-consistency lag.
