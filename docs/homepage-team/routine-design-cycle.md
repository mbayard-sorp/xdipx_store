...
protected-path classifier finds nothing sensitive in the diff. The protected list is **cost-only**
(owner direction 2026-08-19): team valves and spend controls, the enforcement core
(`github.server.ts`, `release-engine.server.ts`, `migration-classify.server.ts`, `.github/**`),
secrets, the checkout probe, the deploy-critical build scripts, and a non-additive migration (a
`db/migrations/**` file with anything beyond `ADD COLUMN IF NOT EXISTS` / `CREATE TABLE IF NOT
EXISTS` / `CREATE INDEX IF NOT EXISTS`, or a modified/renamed migration file). Those stop and email
you; only you merge those. Checkout and cart code, auth and session, `db/schema.ts`, `vercel.json`,
and `package.json` are no longer protected — they are ordinary code PRs covered by CI, QA, and
post-deploy smoke with automatic revert. **The team still cannot merge its own code.** That remains the enforcement of the "gate code" decision, and `release_engine_enabled`
