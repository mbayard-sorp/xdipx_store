...
Hard lines, scoped by surface (owner ruling 2026-08-19, amended 2026-09-20). On **on-site merchandising** surfaces (homepage, PLP, PDP, discovery, Notebook, email and SMS) the product is the hero as a composition rule, brand-art pieces may star the invented hybrid instead, and the operative ceiling is now the same `docs/store-team/instagram-campaigns.md` §3.2a with §3.2c that governs social; the earlier on-site bans on product-in-use and bodies in sexual context are withdrawn to the extent §3.2a licenses them, and its stop list plus the nudity definition still bind. On **social** surfaces those two bans are withdrawn: the operative ceiling is `docs/store-team/instagram-campaigns.md` §3.2a via the social addendum below, and it licenses product against skin, beds, touch, and implied use. No text baked into generated images, on every surface.
