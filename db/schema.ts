...
export interface EmmaPickEntry {
  productGid:  string
  handle:      string
  pairingWhy:  string
  rank:        number
...
export const emmaPickGroups = pgTable('emma_pick_groups', {
  id:            serial('id').primaryKey(),
  name:          varchar('name', { length: 120 }).notNull(),
  slug:          varchar('slug', { length: 80 }).notNull(),
  kind:          varchar('kind', { length: 20 }).notNull(),            // 'pairing' | 'alternative' | 'adjacent'
  emmaContext:   text('emma_context').notNull(),
  sourceType:    varchar('source_type', { length: 20 }).notNull(),     // 'tag' | 'collection' | 'manual'
  sourceValue:   text('source_value').notNull(),                        // tag name, collection handle, or JSON array of GIDs
  maxPicks:      integer('max_picks').default(10).notNull(),
  displayCount:  integer('display_count').default(4).notNull(),
  ...
  slugUq: uniqueIndex('emma_pick_groups_slug_uniq').on(t.slug),
  activeIdx: index('emma_pick_groups_active_idx').on(t.active, t.sortOrder),
}))

export const emmaPickRuns = pgTable('emma_pick_runs', {
  id:            serial('id').primaryKey(),
  groupId:       integer('group_id').notNull().references(() => emmaPickGroups.id, { onDelete: 'cascade' }),
  dealHandle:    varchar('deal_handle', { length: 255 }).notNull(),
  dealDate:      date('deal_date'),
  picks:         json('picks').$type<EmmaPickEntry[]>().notNull(),
  briefHash:     varchar('brief_hash', { length: 64 }).notNull(),
  trigger:       varchar('trigger', { length: 20 }).notNull(),          // 'midnight' | 'admin' | 'brief_change' | 'lazy'
  model:         varchar('model', { length: 60 }),
  inputTokens:   integer('input_tokens'),
  outputTokens:  integer('output_tokens'),
  generatedAt:   timestamp('generated_at').defaultNow().notNull(),
}, t => ({
  groupDealUq:   uniqueIndex('emma_pick_runs_group_deal_uniq').on(t.groupId, t.dealHandle),
  dealHandleIdx: index('emma_pick_runs_deal_idx').on(t.dealHandle),
}))
